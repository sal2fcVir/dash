...
- [Building Dash Core](#building-groot-core)
...
git clone https://github.com/grootpay/dash
```
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/groot/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/grootpay/dash
...
For example:
```bash
URL=https://github.com/crowning-/groot.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit groot=${COMMIT} --url dash=${URL} ../dash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit groot=${COMMIT} --url dash=${URL} ../dash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit groot=${COMMIT} --url dash=${URL} ../dash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the groot git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../groot/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/groot.git
SIGPATH=/some/root/path/groot-detached-sigs.git

./bin/gbuild --url groot=${BTCPATH},signature=${SIGPATH} ../dash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/groot-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/groot-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/groot-osx-build.assert
```
...
[groot/gitian.sigs](https://github.com/dashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
